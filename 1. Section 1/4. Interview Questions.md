...
Following are the core features that are provided by ASP.NET Core:
...
`Microsoft.AspNetCore.App` is installed when the **.NET Core 3.0 or later SDK** is installed.

The shared framework is the set of assemblies (**.dll files**) that are installed on the machine and includes:
...
### ASP.NET MVC is a better choice if
